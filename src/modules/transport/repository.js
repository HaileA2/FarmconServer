...
async function createAssignment({ order_id, transporter_id }) {
  const result = await query(
    'INSERT INTO delivery_assignments (order_id, transporter_id) VALUES ($1, $2) RETURNING *',
...
async function findByOrder(order_id) {
  const result = await query(
    ...
    'WHERE da.order_id = $1 ORDER BY da.created_at DESC',
    [order_id]
  );
  return result.rows;
}
...
async function findByTransporter(transporter_id) {
  const result = await query(
    ...
    'JOIN orders o ON o.order_id = da.order_id ' +
    'JOIN products p ON p.product_id = o.product_id ' +
    'WHERE da.transporter_id = $1 ORDER BY da.created_at DESC',
    [transporter_id]
  );
  return result.rows;
}
...
async function findActiveByOrder(order_id) {
  const result = await query(
    "SELECT * FROM delivery_assignments WHERE order_id = $1 AND status NOT IN ('Delivered','Failed') LIMIT 1",
    [order_id]
  );
  return result.rows[0] || null;
}
